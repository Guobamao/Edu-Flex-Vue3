<template>
    <el-container>
        <el-aside width="200px" class="calendar-aside">
            <el-checkbox v-model="showAllGoal" :true-value="1" :false-value="0" @change="handleShowAllGoal"
                style="margin-left: 8px;">查看全部</el-checkbox>
            <hr />
            <el-checkbox-group v-model="checkBoxValue" class="checkbox-group">
                <el-checkbox v-for="item in goalColorMap" :key="item.goalId" :label="item.goalName"
                    :title="item.goalName" :value="item.goalId" :checked="showAllGoal === 1">
                    <div class="checkbox-item">
                        <div class="goal-color"
                            :style="{ backgroundColor: checkBoxValue.includes(item.goalId) ? item.color : '' }"></div>
        ...
        <el-main class="calendar-container">
            <el-switch v-model="showWorkWeek" active-text="显示周末" inactive-text="隐藏周末" :active-value="false"
                :inactive-value="true" @change="changeWorkWeek" />
            ...
            <!-- 创建计划 popover -->
            <el-popover :virtual-ref="triggerRef" virtual-triggering :title="title" :width="400"
                ...
                <el-form ref="planRef" :model="form" :rules="rules" label-width="80px">
                    <el-form-item label="学习目标" prop="goalId">
                        <el-select v-model="form.goalId" placeholder="请选择学习目标">
                            <el-option v-for="item in goalList" :key="item.id" :label="item.goalName"
                                :value="item.id" />
                        </el-select>
                    </el-form-item>
                    ...
                    <el-form-item label="开始时间" prop="startTime">
                        <el-date-picker v-model="form.startTime" type="datetime" placeholder="选择开始时间"
                            @change="handleStartTimeChange" style="width: 100%;" />
                    </el-form-item>
                    ...
                    <el-form-item label="提醒时间" prop="reminderTime" v-if="form.remind == '1'">
                        <el-date-picker clearable v-model="form.reminderTime" type="datetime"
                            value-format="YYYY-MM-DD HH:mm:ss" placeholder="请选择提醒时间">
                        </el-date-picker>
                    </el-form-item>
                    <el-form-item>
...
const backgroundColors = [
    '#ff8b88', '#ffd966', '#ffe680',
    '#fff0a3', '#d9e778', '#a4d8a7',
    '#77dd77', '#a1caf1', '#8da9c2',
    '#b2a9c2', '#d9a1c2', '#ffb2a9',
    '#ff8b88', '#ffd966', '#ffe680',
    '#fff0a3', '#d9e778', '#a4d8a7'
];
...
function handleDelete() {
    proxy.$modal.confirm('确认删除该学习计划？').then(() => {
...
function setDateRangeText() {
    ...
    switch (calendar.value.getViewName()) {
        case 'month':
            ...
            return;
        case 'day':
            dateRangeText.value = `${date.getFullYear()}.${String(date.getMonth() + 1).padStart(2, '0')}.${String(date.getDate()).padStart(2, '0')}`;
            return;
        default:
            dateRangeText.value = `${startYear}.${String(date.getMonth() + 1).padStart(2, '0')}.${String(start.getDate()).padStart(2, '0')} 
            ~ 
            ${startYear !== endYear ? `${endYear}.` : ''}${String(end.getMonth() + 1).padStart(2, '0')}.${String(end.getDate()).padStart(2, '0')}`;
    }
}
