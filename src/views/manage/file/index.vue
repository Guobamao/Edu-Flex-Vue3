<template>
  <div class="app-container">
    <el-form
      ...
      <el-form-item label="文件所属类型" prop="type">
        <el-select
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button type="warning" plain icon="Download" @click="handleExport" v-hasRole="['admin']"
          >导出</el-button
        >
      </el-col>
    ...
    <el-table v-loading="loading" :data="fileList" border @selection-change="handleSelectionChange">
      ...
      <el-table-column label="文件名" align="center" prop="originalFilename" show-overflow-tooltip>
      ...
      <el-table-column label="库文件名" align="center" prop="filename" show-overflow-tooltip />
      <el-table-column label="文件归属类型" align="center" prop="objectId">
        <template #default="scope">
          <dict-tag :options="file_type" :value="scope.row.objectId" />
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="View" @click="handlePreview(scope.row)"
            >预览</el-button
          >
          <el-button link type="primary" icon="Download" @click="downloadFile(scope.row)"
            >下载</el-button
          >
          <el-button
            link
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="fileRef" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="库文件名" prop="filename">
        ...
        <el-form-item label="文件归属类型" prop="objectId">
          <el-select v-model="form.objectId" placeholder="请选择文件归属类型">
            <el-option
...
const { material_type, file_type } = proxy.useDict("material_type", "file_type");
...
const data = reactive({
  ...
  rules: {
    originalFilename: [{ required: true, message: "文件名不能为空", trigger: "blur" }],
    filename: [{ required: true, message: "库文件名不能为空", trigger: "blur" }],
    ...
    objectId: [{ required: true, message: "文件归属类型不能为空", trigger: "blur" }],
  },
});
...
function handleDelete(row) {
  ...
  proxy.$modal
    .confirm('是否确认删除资源管理编号为"' + _ids + '"的数据项？')
    .then(function () {
      return delFile(_ids);
    })
    .then(() => {
...
function downloadFile(row) {
  ...
  proxy.download("/common/download/" + row.id, {}, row.originalFilename);
}
